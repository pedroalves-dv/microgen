...
export default function Home() {
  ...
  async function handleSubmit(e) {
    e.preventDefault()
    setLoading(true); setError(null); setBrief(null); setArticle(null)
    try {
  ...
  async function handleGenerateArticle() {
    setGenerating(true); setError(null); setArticle(null)
    try {
        ...
        <div style={{ maxWidth: 760, margin: '0 auto' }}>
          ...
          <form onSubmit={handleSubmit}>
            <div style={{
              ...
              boxShadow: '0 1px 0 rgba(0,0,0,0.02), 0 16px 40px -24px rgba(0,0,0,0.12)',
            }}>
              ...
              <button disabled={loading} style={{
                ...
                opacity: loading ? 0.5 : 1, display: 'inline-flex', alignItems: 'center', gap: 8,
              }}>
                {loading ? (<><Spinner/> Generating…</>) : 'Generate →'}
              </button>
            </div>
            <div className="mono" style={{
              marginTop: 12, fontSize: 11.5, color: 'var(--mute)',
              display: 'flex', gap: 22,
            }}>
          ...
          {!brief && !loading && !error && (
            <div style={{ marginTop: 96 }}>
              ...
              }}>SEO content engine</div>
              <h1 style={{
                margin: 0, fontSize: 84, lineHeight: 0.94,
                letterSpacing: '-0.05em', fontWeight: 400, color: 'var(--ink)',
              }}>
                One keyword.<br/>
                <span className="serif" style={{ fontStyle: 'italic' }}>A finished article.</span>
              </h1>
              <p style={{
                marginTop: 28, fontSize: 18, color: 'var(--ink-2)', lineHeight: 1.55,
                letterSpacing: '-0.005em', maxWidth: 560,
              }}>
                Microgen turns a single search phrase into a publish-ready brief —
                title, meta, H2s, gaps, internal links — and writes the full draft
                on top of it.
              </p>
            </div>
          )}
          ...
          {/* Loading state */}
          {loading && (
            <div style={{ marginTop: 64, textAlign: 'center', color: 'var(--mute)' }}>
              <div className="mono" style={{ fontSize: 12, letterSpacing: '0.08em', textTransform: 'uppercase' }}>
                Drafting your brief
              </div>
              <p className="serif" style={{
                margin: '14px auto 0', fontSize: 24, fontStyle: 'italic',
                color: 'var(--ink)', maxWidth: 480, lineHeight: 1.35,
              }}>
                Searching intent signals, mapping out an outline, and finding the angles other articles missed.
              </p>
            </div>
          )}
          ...
          {error && (
            <div style={{
              ...
              borderRadius: 10, color: 'var(--ink)', fontSize: 13.5, lineHeight: 1.5,
            }}>
              <div className="mono" style={{ fontSize: 11, color: 'var(--accent)', letterSpacing: '0.08em', textTransform: 'uppercase', marginBottom: 4 }}>
          ...
          {/* Brief */}
          {brief && <Brief brief={brief} onGenerate={handleGenerateArticle} generating={generating}/>}
...
function Brief({ brief, onGenerate, generating }) {
  return (
    <div style={{ marginTop: 56, position: 'relative' }}>
      ...
      <h2 style={{
        margin: '6px 0 0', fontSize: 44, fontWeight: 400,
        letterSpacing: '-0.035em', lineHeight: 1.06, color: 'var(--ink)',
      }}>{brief.title}</h2>

      {brief.meta_description && (
        <p style={{
          margin: '20px 0 0', fontSize: 17, color: 'var(--ink-2)',
          lineHeight: 1.55, letterSpacing: '-0.005em', maxWidth: 640,
        }}>{brief.meta_description}</p>
      )}
      ...
      {Array.isArray(brief.h2_headings) && brief.h2_headings.length > 0 && (
        <section style={{ marginTop: 56 }}>
          <SectionHead title="Outline" meta={`${brief.h2_headings.length} sections`}/>
          <div style={{ borderTop: '1px solid var(--line)' }}>
            {brief.h2_headings.map((h, i) => (
              <div key={i} style={{
                display: 'grid', gridTemplateColumns: '40px 1fr',
                alignItems: 'baseline', gap: 16, padding: '18px 0',
                borderBottom: '1px solid var(--line)',
              }}>
                <span className="mono" style={{ color: 'var(--mute-2)', fontSize: 12 }}>
                  {String(i + 1).padStart(2, '0')}
                </span>
                <span style={{ fontSize: 17, color: 'var(--ink)', letterSpacing: '-0.015em', lineHeight: 1.35 }}>{h}</span>
              </div>
            ))}
          </div>
        </section>
        ...
        <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 48, marginTop: 56 }}>
          {brief.target_audience && (
            <div>
              ...
              <p style={{ margin: '8px 0 0', fontSize: 15, color: 'var(--ink-2)', lineHeight: 1.55, letterSpacing: '-0.005em' }}>
                {brief.target_audience}
              </p>
            </div>
          )}
      ...
      {Array.isArray(brief.internal_linking_suggestions) && brief.internal_linking_suggestions.length > 0 && (
        <section style={{ marginTop: 48 }}>
          ...
          <ul className="mono" style={{ margin: '20px 0 0', padding: 0, listStyle: 'none' }}>
      ...
      {brief.cta_suggestion && (
        <section style={{ marginTop: 56 }}>
          ...
          <p className="serif" style={{
            margin: '12px 0 0', fontSize: 26, fontStyle: 'italic',
            lineHeight: 1.3, color: 'var(--ink)', letterSpacing: '-0.01em',
            maxWidth: 640,
          }}>“{brief.cta_suggestion}”</p>
        </section>
      )}
      ...
      <div style={{
        ...
        <div>
          <div style={{ fontSize: 16, fontWeight: 500, color: 'var(--ink)', letterSpacing: '-0.015em' }}>
        ...
        <button onClick={onGenerate} disabled={generating} style={{
          ...
          {generating ? (<><Spinner/> Generating article…</>) : 'Generate full article →'}
        </button>
      </div>
    </div>
  )
...
function Article({ article }) {
  return (
    <section style={{
      ...
      <div className="mono" style={{ fontSize: 11, color: 'var(--mute)', letterSpacing: '0.1em', textTransform: 'uppercase' }}>
...
function SectionHead({ title, meta }) {
  return (
    <div style={{ display: 'flex', alignItems: 'baseline', justifyContent: 'space-between' }}>
      <h3 style={{ margin: 0, fontSize: 18, fontWeight: 500, color: 'var(--ink)', letterSpacing: '-0.015em' }}>
        {title}
      </h3>
      {meta && <span className="mono" style={{ fontSize: 11, color: 'var(--mute)' }}>{meta}</span>}
    </div>
  )
}
...
function Spinner() {
  return (
    <svg width="14" height="14" viewBox="0 0 24 24" fill="none" style={{ animation: 'spin 1s linear infinite' }} aria-hidden="true">
